...
class VirtualFixtureDemo(Node):
    def __init__(self):
        ...
        # load stanford bunny from open3d examples
    ...
    def enforce(self, sphere_target_center,old_sphere_center, sphere_radius):

        if np.linalg.norm(sphere_target_center - old_sphere_center) > 0.0001:
            ray_start = old_sphere_center
            ray_end = sphere_target_center
            ray_direction_normalized = ray_end - ray_start / np.linalg.norm(ray_end - ray_start)
            self.ray_line.points = o3d.utility.Vector3dVector([ray_start, ray_end])
            self.viz.update_geometry(self.ray_line)
        
        distance, closest_point = self._point_to_mesh_distance(old_sphere_center, self.surface)
        if distance - 0.005 < sphere_radius:
            direction = old_sphere_center - closest_point
            direction /= np.linalg.norm(direction)
            # sphere center is casted to the surface
            # vf_center = closest_point + direction * sphere_radius

            # nullify the component of the target direction parallel to direction
            target_direction = sphere_target_center - old_sphere_center
            target_direction = target_direction - np.dot(target_direction, direction) * direction
            target_direction /= np.linalg.norm(target_direction)
            vf_center = closest_point + target_direction * sphere_radius 
            return vf_center
        else:
            return sphere_target_center
    ...
    def _run_main_loop(self):
        ...
        while running:
            ...
            self.vf = self._enforce_virtual_fixture(self.sphere_target_center, self.surface, POINT_RADIUS)
            self.sphere_center = self.old_sphere_center + (self.vf - self.old_sphere_center) * 0.1
